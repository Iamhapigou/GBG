...
import random
import torchtext
from experiment.dataset_utils import check, separate_data, split_data, save_file
from experiment.dataset_utils import tokenizer
...
max_len = 200
max_tokens = 32000
dir_path = os.path.join(os.path.dirname(os.path.abspath('__file__')), 'AGNews/')

class AGNewsDataset(Dataset):
    def __init__(self, csv_path):
        df = pd.read_csv(csv_path)
        self.labels = df.iloc[:, 0].astype(int).tolist()
...
def generate_dataset(dir_path, num_clients, niid, balance, partition):
    ...
    # Get AG_News data
    #os.makedirs(os.path.join(dir_path, "rawdata"), exist_ok=True)
    #trainset, testset = torchtext.datasets.AG_NEWS(root=dir_path+"rawdata")

    trainset = AGNewsDataset(os.path.join(dir_path, "rawdata/ag_news_csv/train.csv"))
    testset = AGNewsDataset(os.path.join(dir_path, "rawdata/ag_news_csv/test.csv"))
