...
from experiment.server.serveravg_kd import FedAvg_KD
from experiment.server.serverprox_kd import FedProx_KD
from experiment.server.servergroup_kd import FedGroup_KD
from experiment.server.servergrouprox_kd import FedGrouprox_KD
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if args.dataset == "MNIST":
            ...
            args.model = Cifar100_models.CIFAR100_MLP().to(args.device)
    ...
    if args.algorithm == "FedAvg_KD":
        server = FedAvg_KD(args, i)

    elif args.algorithm == "FedProx_KD":
        server = FedProx_KD(args, i)

    elif args.algorithm == "FedGroup_KD":
        server = FedGroup_KD(args, i)

    elif args.algorithm == "FedGrouprox_KD":
        server = FedGrouprox_KD(args, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-algo', "--algorithm", type=str, default="FedGroup_KD")
    ...
    parser.add_argument('-nc', "--num_clients", type=int, default=15,
                        help="Total number of clients")
    parser.add_argument('-ng', "--num_groups", type=int, default=3,
                        help="Total number of groups")
    parser.add_argument('-lr', "--local_learning_rate", type=float, default=0.01,
                        help="Local learning rate")
    parser.add_argument('-alp', "--alpha", type=float, default=0.5)
